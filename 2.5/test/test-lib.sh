...
# CID_FILE_DIR=$($MKTEMP_EXEC --suffix=<container>_test_cidfiles -d)
...
function ct_obtain_input() {
  ...
  local output=$($MKTEMP_EXEC "/var/tmp/test-input-XXXXXX$extension")
  if [ -f "${input}" ] ; then
    cp -f "${input}" "${output}"
  elif [ -d "${input}" ] ; then
    rm -f "${output}"
...
ct_test_response() {
  ...
  local status
  local response_code
  local response_file=$($MKTEMP_EXEC /tmp/ct_test_response_XXXXXX)
  while [ ${attempt} -le ${max_attempts} ]; do
...
ct_s2i_build_as_df()
{
    ...
    df_name=$($MKTEMP_EXEC -p "$tmpdir" Dockerfile.XXXX)
    pushd "$tmpdir"
